...
from base.utility import check_email_or_phone, send_email, send_phone_code, check_user_type
...
class SignUpSerializer(serializers.ModelSerializer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['reset_password'] = serializers.CharField(max_length=120, required=True)
...
class LoginSerializer(TokenObtainPairSerializer):
    ...
    def auth_validate(self, data):
        ...
        if current_user is not None and current_user.auth_status in [NEW, CODE_VERIFIED]:
            raise ValidationError(
                {
                    'success': False,
                    "message": "You are don't already logged in"
                }
            )
        ...
        if user is not None:
            ...
            raise ValidationError(
                {
                    'success': False,
                    "message": "Sorry, login or password is incorrect. Please try again."
                }
            )
    ...
    def validate(self, data):
        ...
        if self.user.auth_status not in [DONE, PHOTO_STEP]:
            raise PermissionDenied("You don't login. You don't have permission to do that.")
